...
export async function register(req, res) {
  ...
  if (!phoneNumber) { //if phone number is not valid
  ...
  const exsist = await User.findOne({ //check if user already exists
    $or: [ 
      { 
        email: newEmail, 
      },
  ...
  if (exsist) { //if user already exists
    return res.status(Http.BAD_REQUEST).json({
      success: false,
  ...
  return Bcrypt.hash(req.body.password, 10, (err, hash) => {
    if (err) {
      return res.status(Http.INTERNAL_SERVER_ERROR).json({ //if error
        success: false,
    ...
    User.create(newUser) //save new user
      .then((result) => {
        ...
        return res.status(Http.CREATED).json({ //if user created
          success: true,
...
export function getUserList(req, res) {
  User.find({}) //get all users 
    ...
    .then((result) => { 
      if (result.length === 0) { //if no users found
        return res.status(Http.NOT_FOUND).json({
          success: false,
          message: "Users not found", //return error message
        });
      }
      return res.status(Http.OK).json({ //if users found
        success: true,
        message: "Users found",
        users: result,
      });
...
export function login(req, res) {
    ...
    User.findOne({ //find user by email
        ...
        .then((result) => { 
        if (!result) {
            return res.status(Http.NOT_FOUND).json({ //if user not found
            success: false,
            message: "User not found",
            });
        }
        return Bcrypt.compare(req.body.password, result.password, (err, data) => { //compare password
            if (err) {
            return res.status(Http.INTERNAL_SERVER_ERROR).json({
                success: false,
                message: "Server error",
            ...
            if (!data) { //if password not correct
            return res.status(Http.BAD_REQUEST).json({
                success: false,
                message: "Invalid password", //return error message
            });
            }
            return res.status(Http.OK).json({ //if user logged in
            success: true,
...
export async function updateUser(req, res) {
    ...
    req.body.password ? newPassword = Bcrypt.hashSync(req.body.password, 10) : newPassword = userData.password; //if password is provided, hash it, else use old password
     
    User.findByIdAndUpdate(id, { //update user data
        name: req.body.name,
        email: req.body.email,
        phone: req.body.phone,
        ...
        .then((result) => {
        if (!result) { //if user not found
            return res.status(Http.NOT_FOUND).json({
            success: false, //return error message
            message: "User not found",
            });
            }
            return res.status(Http.OK).json({ //if user updated
                success: true, 
            });
        })
        .catch((err) => {
